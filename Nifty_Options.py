...
oi_filename = os.path.join(".", "Data/nifty_{0}.json".format(datetime.now().strftime("%d%m%y")))
...
def fetch_oi(df):
    ...
    while(1):
        try:
            nifty_url = requests.get(n_url, headers=headers).json()
    ...
    with open("Data/Datanifty.json", 'w') as files:
